...
export default function LayoutDashboard({ children }: { children: React.ReactNode }) {
    ...
    function Menu({ menuItems }: { menuItems: Array<itemMenu> }) {
        return menuItems.map((item, index) => (
            <Accordion key={item.link + index} collapsible={!!item.submenus?.length}>
                <Accordion.Item value={item.title}>
                    <Accordion.Item.Trigger collapsible={!!item.submenus?.length} className="flex justify-between items-center ">
    ...
    return (
        <div>
            ...
            <div className="p-4 sm:ml-64 bg-gray-200 w-screen h-screen">
                {children}
            </div>
        </div >
    )
